...
export const defaultTheme = {
  ...
  typography: {
    ...
    fontSize: "txl", // will be overridden dynamically
    lineHeight: "normal",
  },
  layout: {
    maxWidth: "900px",
...
export function ThemeProvider({ children }) {
  ...
  // Helper to determine font size based on screen width
  const getResponsiveFontSize = () => {
    if (typeof window === "undefined") return "16px"
    const width = window.innerWidth
    if (width < 640) return "16px"      // small/mobile
    if (width < 1024) return "20px"     // medium/tablet
    return "24px"                        // large/desktop
  }
  ...
  useEffect(() => {
    ...
    if (savedTheme) {
      try {
        ...
        initialTheme = { ...defaultTheme, ...parsedTheme }
      } catch (error) {
        console.error("Error parsing saved theme:", error)
      }
    }

    // Override font size responsively
    initialTheme.typography.fontSize = getResponsiveFontSize()
  ...
  const resetTheme = () => {
    const reset = { ...defaultTheme, typography: { ...defaultTheme.typography, fontSize: getResponsiveFontSize() } }
    setTheme(reset)
...
function getFontSizeValue(size) {
  ...
  // If a value is already px, return it directly
